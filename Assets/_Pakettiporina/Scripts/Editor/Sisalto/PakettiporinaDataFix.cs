...
namespace Pakettiporina.EditorTools
{
    ...
    public static class PakettiporinaDataFix
    {
        ...
        [MenuItem("Pakettiporina/22a - Korjaa kategoriat ja kosmeettisuus")]
        public static void FixCategories()
        {
            ...
            // varinimet -> Maali
            var colorNames = new[] { "punainen", "sininen", "vihrea", "vihre�", "keltainen", "musta", "valkoinen" };
        ...
        [MenuItem("Pakettiporina/22b - Tasapainota osien arvot")]
        public static void Rebalance()
        {
            ...
            var table = new Dictionary<string,(int v,int p,int k,int ke,int ky)>
            {
                ...
                {"Urheilukori",  (0, 5, 20,-8, 0)},   // kevyt & ketter�, hauras
                {"Maastokori",   (0, 8,-10,20, 0)},   // kest�v� & pit�v�, raskas
                {"Kevytkori",    (0,-4,28,-12,0)},    // eritt�in kevyt, hauras
                // RENKAAT: pito vs keveys
                {"Katurenkaat",  (0, 6, 14, 0, 0)},   // nopea kadulla
                {"Maastorenkaat",(0,24,-14, 4, 0)},   // huippupito, hidas
                {"Talvirenkaat", (0,16, -6, 0,10)},   // pito + v�h�n kylm��
                // MOOTTORIT: voima vs keveys
                {"Peruskone",    (14, 0, 0, 4, 0)},   // tasainen
                {"S�hk�moottori",(22, 0, 8, 0, 0)},   // voimaa & kevyt
                {"Turbomoottori",(34, 0,-10,-4, 0)},  // huippuvoima, raskas
                // JOUSET: pito vs kesto
                {"Pehme�t jouset",(0,18, 0, 4, 0)},   // pehme� pito
                {"Kovat_jouset", (0,-4, 6,16, 0)},    // kova, kest�v�
                {"Kisajouset",   (0,22, 6,-6, 0)},    // kisapito, hauras
                // LIS�T
                {"Kylmalaatikko",(0, 0,-6,-4,80)},    // kylm�, painava
                {"Tavaratila",   (0, 0,-8,18, 0)},    // kest�v�, raskas
            };
        ...
        // ---------- 8c: luo lis�� osia ----------
        [MenuItem("Pakettiporina/22c - Luo lisaosia")]
        public static void AddParts()
        {
            ...
            var recipes = new (string name, PartCategory cat, int v,int p,int k,int ke,int ky, bool cos)[]
            {
                ...
                ("Kuormakori",  PartCategory.Kori,     0, 4,-12, 24, 0, false),  // kest�v� ty�kori
                ("Hiekkarenkaat",PartCategory.Renkaat, 0,20, -8,  2, 0, false),  // hiekka/maasto
                ("Rekkakone",   PartCategory.Moottori,26, 0,-14, 6, 0, false),   // v��nt��
                ("Kisajouset2", PartCategory.Jouset,   0,20, 8, -8, 0, false),   // (vaihtoehto)
                ("Lampolaatikko",PartCategory.Lisat,   0, 0,-6, -2,-30, false),  // pit�� l�mpim�n� (neg kylmyys)
        ...
        // ---------- 8: kaikki kerralla ----------
        [MenuItem("Pakettiporina/22 - KORJAA KAIKKI (kategoriat + tasapaino + lisaosat)")]
